...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
    const rawResponse = await fetch('https://rest-jans-wian.azurewebsites.net/AdminContact/request', {
        method: 'POST',
        headers: {
    ...
    if (rawResponse.status == 200) {
        const response = await fetch('https://rest-jans-wian.azurewebsites.net/AdminContact/Admins', {
            method: 'get',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
