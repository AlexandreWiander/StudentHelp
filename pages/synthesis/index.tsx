    ...
    if (files != null) {
      ...
      if (one != undefined) {
        if (addClass != "0") {
          ...
          toast.error(
            "Veuillez choisir un cours en utilisant le filtre de cours !",
            {
              position: "top-center",
              autoClose: 5000,
              hideProgressBar: false,
  ...
  return (
    <div className="h-screen grid grid-cols-3">
      <div className="font-face-pg flex flex-col items-center col-span-2">
        ...
        <div className="grid grid-cols-4 overflow-y-scroll h-1/2 scrollbar-thin scrollbar-thumb-blueTheme scrollbar-track-blue-300">
          {synthesis.map((synthese) => (
            <div
              key={synthese["id"]}
              className="w-2/3 col-span-1 h-auto flex flex-col items-center m-4 p-4 border border-blueTheme bg-white rounded-lg shadow-md hover:bg-gray-100 h-auto"
            >
      ...
      <div className="flex flex-col items-center mt-2 font-face-pg">
        <p className="text-2xl font-extrabold tracking-tight leading-none mt-2 text-center">
          Ajouter une nouvelle synthèse:
        </p>
        ...
        <div className="flex">
          <input
            ...
            id="file"
            type="file"
            accept=".pdf,.doc,.docx,.txt,.xls,.xlsx,.png,.jpg,.jpeg,.gif,.csv"
          />
          <button
            className="bg-blueTheme text-white font-bold m-2 px-4 rounded"
            onClick={addFile}
          >
            Ajouter
          </button>
        ...
        <img
          src={cat.src}
          className={`${styles["loading"]} m-2 w-20`}
          id="loadingGif"
        ></img>
        <p className="text-2xl font-extrabold tracking-tight leading-none mt-2 text-center">
          Mes synthèses:
        ...
        <div className="grid grid-cols-2 overflow-y-scroll h-auto scrollbar-thin scrollbar-thumb-blueTheme scrollbar-track-blue-300 justify-center">
          {mySynthesis.map((synthese) => (
            <div
              key={synthese["id"]}
              className=" span-1 flex justify-center items-center m-4 py-2 px-10 border text-center border-blueTheme bg-white rounded-lg shadow-md hover:bg-gray-100 h-auto"
            >
              <h5 className="text-xs font-bold text-center">
                {synthese["name"]}
              </h5>
              <button id={synthese["id"]} onClick={deleteSynthese}>
                <img src={"images/block.png"} className="w-8 m-2" />
              </button>
            </div>
          ))}
        </div>
